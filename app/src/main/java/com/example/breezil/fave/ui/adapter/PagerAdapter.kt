...
class PagerAdapter(fm: FragmentManager, internal var context: Context) : FragmentStatePagerAdapter(fm) {
    ...
    override fun getPageTitle(position: Int): CharSequence? {
        ...
        when (position) {
            preferred_Position -> return resources.getString(R.string.preferred_fragment)

            headline_Position -> return resources.getString(R.string.head_line_fragment)
            bookmark_Position -> return resources.getString(R.string.book_mark_fragment)
            else -> return "none"
        }
    }
