...
constructor(private var endpointRepository: EndPointRepository,
    ...
    override fun onPaginationSuccess(response: ArticleResult, callback: PageKeyedDataSource.LoadCallback<Int, Articles>, params: PageKeyedDataSource.LoadParams<Int>, results: MutableList<Articles>) {
        if (response.articles != null && response.articles!!.size > ZERO) {
            results.addAll(response.articles!!)
